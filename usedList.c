...
void usedList_insert(UsedList* l, int id, int start, int end, int duration) { //insert into UsedList based on size

    UsedListNode* temp = l->head;
    UsedListNode* prev;

    UsedListNode* new = malloc(sizeof (UsedListNode));
    new->id = id; //assing an increasing id to processes
    l->count++;
    new->start = start;
    new->end = end;
...
void removeFinishedProcess(UsedList* ul, SpaceList* sl, FILE* logfile, int id) { //check if any process has finished

    UsedListNode* temp = ul->head;
    SpaceListNode* slnode;

    while (temp != NULL) {
        if (temp->id == id) { //if process has finished
            ...
            usedList_delete(ul, victim->id); //remove from usedList
        } else {
            temp = temp->next;
        }
    }
